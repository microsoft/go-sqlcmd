...
var InvalidServerName = ArgumentError{
	Parameter: "server",
	Rule:      localizer.Sprintf("server must be of the form [tcp]:server[[/instance]|[,port]]"),
}
...
func InvalidVariableValue(variable string, value string) *VariableError {
	return &VariableError{
		Variable:      variable,
		MessageFormat: localizer.Sprintf("The environment variable: '%s' has invalid value: '"+strings.ReplaceAll(value, `%`, `%%`)+"'.", variable),
	}
}
...
func InvalidFileError(err error, filepath string) error {
	return &FileError{
		err:  errors.New(localizer.Sprintf(ErrorPrefix + " Error occurred while opening or operating on file " + filepath + " (Reason: " + err.Error() + ").")),
		path: filepath,
	}
}
