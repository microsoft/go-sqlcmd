...
type SQLCmdArguments struct {
	...
	ApplicationIntent           string            `short:"K" default:"" enum:",ReadOnly" help:"Declares the application workload type when connecting to a server. The only currently supported value is ReadOnly. If -K is not specified, the sqlcmd utility will not support connectivity to a secondary replica in an Always On availability group."`
	EncryptConnection           string            `short:"N" default:"false" enum:"false,true,disable" help:"This switch is used by the client to request an encrypted connection."`
}
