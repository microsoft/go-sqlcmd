...
func run(vars *sqlcmd.Variables, args *SQLCmdArguments) (int, error) {
	...
	s.Format = sqlcmd.NewSQLCmdDefaultFormatter(
		vars,
		args.TrimSpaces,
		args.getControlCharacterBehavior(),
		sqlcmd.WithRawErrors(args.RawErrors),
		sqlcmd.WithRegionalSettings(args.UseRegionalSettings),
	)
	if args.OutputFile != "" {
