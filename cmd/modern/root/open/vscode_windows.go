...
func (c *Vscode) displayPreLaunchInfo() {
	...
	output.Info(localizer.Sprintf("1. Install the MSSQL extension if not already installed (Ctrl+Shift+X, search 'SQL Server (mssql)')"))
	output.Info(localizer.Sprintf("2. Open the Command Palette (F1 or Ctrl+Shift+P)"))
